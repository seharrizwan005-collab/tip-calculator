...
- My original course code used a dropdown
...
- It helped rewrite the calculate function to support live updates and add the custom tip input logic
...
- The custom tip field doesn't handle edge cases like typing just `.` — `parseFloat(".")` silently returns NaN and results go blank with no clear message
